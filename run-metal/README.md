...
3)  Dynamically writes the script to run METAL: `modules/write_metal_script.py`
    * METAL is default to run inv-var weigthed meta-analysis, genomic correction will be applied (ON, OFF, or VALUE) based on the user supplied value in the config file
    * If you are curious what this file looks like, it is written here: `<output_dir>/run-metal/<pehnotype>.metal`
4) Runs METAL: `~line 319 in Snakefile`
    * METAL binary is included in this repository
5) Summarize metal outputs: `modules/summary.py`
...
6) Create Manhattan & QQ Plots from metal output: `modules/plots.py`
...
7) Run LDSC Regression: `modules/prep_for_ldsc.py`
...
8) Caculates the most significant variant per non-overlapping genomic region: `modules/significnt_loci.py`
...
9) Run ANNOVAR to attach the nearest gene to list of significant variants: `modules/prep_for_annovar.py`
...
10) Create the final summary: `modules/add_genes.py`
...
|hapmap|w_hm3.snplist|
|ref_ld_chr|LD score reference directory|
|w_ld_chr|LD score weights directory|
...
|biobank_min|Minimum number of biobanks that each meta-analyzed variant must be found in to be considered in downstream analysis|
...
|LDSC Matric Results|`<output_path>/cli_ldsc_matrix/<phenotype>.log`|
