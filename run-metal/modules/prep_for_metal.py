...
def prep_qc_files(
    *,
    ...
    # Add lines to test ACAF threshold on AoU Data
    if 'AoUACAF' in output_path.name:
        print(combined_df)
        combined_df = combined_df.filter(pl.col('Aligned_AF')>0.01)
        print(combined_df)
...
def _read_specific_columns(
    *,
    ...
    if (case_count_col == "nan") and (control_count_col == "nan") and (total_count_col != "nan"):
